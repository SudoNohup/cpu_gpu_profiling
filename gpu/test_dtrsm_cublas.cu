...
int main(int argc, char **argv) {
  ...
  */
  ...
  char side ='R', uplo = 'L', trans = 'T', unit = 'N'; 
  int nrhs = n;
  ...
  checkCublas(cublasDtrsm(handle, (side == 'L' ? CUBLAS_SIDE_LEFT : CUBLAS_SIDE_RIGHT), (uplo == ':' ? CUBLAS_FILL_MODE_LOWER : CUBLAS_FILL_MODE_UPPER), (trans == 'N' ? CUBLAS_OP_N : CUBLAS_OP_T), (unit == 'N' ? CUBLAS_DIAG_NON_UNIT : CUBLAS_DIAG_UNIT), n, nrhs, &alpha, d_A, n, d_C, n));
  cudaEventRecord(stop,0);
  cudaEventSynchronize(stop);
