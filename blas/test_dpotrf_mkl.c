...
int main(int argc, char **argv) {
  ...
  */
  ...
  //printf("%f\t", (1.0/3*m*n*k* 1e-6) / mkl_time);
  printf("%f\t", mkl_time);
  ...
  for (ii = 0; ii < nreps; ++ii) {
	...
	dpotrf(uplo, n, A, n);
	mkl_time = endTimer(&time);
	//printf("mkl_time: %g (%g GFLOPS)\n", mkl_time, (1.0/3*m*n*k* 1e-6) / mkl_time);
	sum += mkl_time;
...
//  printf("%f\t", (1.0/3*m*n*k* 1e-6)/ fastest_time);
//  printf("%f\t", (1.0/3*m*n*k* 1e-6)/ slowest_time);
//  printf("%f\t", (1.0/3*m*n*k* 1e-6)/ average_time);
