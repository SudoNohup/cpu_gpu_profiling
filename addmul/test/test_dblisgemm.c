    ...
    ) 
{
  ...
  for ( i = 0; i < n; i ++ ) {
    for ( p = 0; p < k; p ++ ) {
      XE[ i * k + p ] = (double)( rand() % 1000000 ) / 1000000.0;	
    }
  }
...
//    }
  ...
  printf( "%5d\t%5d\t%5d\t%le\t%le\t%5.2lf\t%5.2lf\n", 
      m, n, k, dblisgemm_rectime, ref_rectime, flops / dblisgemm_rectime, flops / ref_rectime );
